...
@traceable(
        ...
        run_type="llm",
        metadata={"ls_provider": config.EMBEDDING_MODEL_PROVIDER,"ls_model_nane": config.GENERATION_MODEL}
)
...
@traceable(name="rag-rag_pipeline")
...
def rag_pipeline_wrapper(question, top_k=5):
    ...
    image_url_list = []
    for rag_used_context in result['answer'].retrieved_context_ids:
        payload = qdrant_client.retrieve(
            collection_name=config.QDRANT_COLLECTION_NAME,
            ids=[rag_used_context.id]

        )[0].payload ## Get all the payload from Qdrant
        image_url = payload.get("first_large_image") # From payload get image url
        price = payload.get("price") # From payload get price
        if image_url:
            image_url_list.append({"image_url": image_url, "price": price, "description": rag_used_context.description}) # the image url for the point to list of image ursl
    return {
